...
def parse_annotated_file(filepath: str) -> dict[str, list[dict]]:
    ...
    while i < len(lines):
        ...
        if line and not re.match(r'^\d+$', line):
            ...
            if re.match(r'^[0-3]$', score_candidate) and show_candidate and episode_candidate:
                # Save previous query
...
def plot_avg_precision_recall(pr_results: dict, output_path: str = RES_FOLDER+"/pr_curve.png"):
...
def save_metrics_per_query(pr_results: dict, dcg_results: dict,
...
def save_metrics_averaged(pr_results: dict, dcg_results: dict,
...
def save_pr_curve_per_query(data: dict, pr_results: dict,
...
def print_summary(pr_results: dict, dcg_results: dict):
    ...
    ndcg10 = {q: dcg_results["per_query"][q]["nDCG"][10] for q in dcg_results["per_query"]}
...
if __name__ == "__main__":
    ...
    print("\nSaving CSVs …")
    save_metrics_per_query(pr_results, dcg_results, RES_FOLDER+"/metrics_per_query.csv")
    ...
    plot_per_query_precision_recall(data, pr_results, out_dir=RES_FOLDER+"/"+PER_QUERY_PLOT_DIR)
